...
# --- 1. Read Excel Data ---
excel_file = 'digital_twin_pushover_data.xlsx'  # Change to your actual file name

# Sheet 1: Wall/material parameters (16 samples)
...
for i, wid in enumerate(wall_ids):
    ...
    df_all_curves.to_csv(f'digital_twin_all_curves_wall_{wid}.csv', index=False)
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(f'pushover_curve_wall_{wid}.png', dpi=300)
    plt.close()
...
plt.tight_layout()
plt.savefig('model_performance_r2.png', dpi=300)
plt.close()
...
plt.grid(True)
plt.tight_layout()
plt.savefig('pushover_curve_all_walls.png', dpi=300)
plt.show()
plt.xlabel('Displacement (mm)')
plt.ylabel('Base Force (kN)')
...
opening_ratio_new = 8.7 # Replace with the actual opening ratio for the new wall
...
# Create a curved (cubic interpolated) version of the predicted curve

# Create a smooth 4th-degree polynomial fit for the predicted curve
...
df_pred_new_bilinear.to_csv('digital_twin_predicted_curve_new_wall_bilinear.csv', index=False)
...
df_pred_new_poly4.to_csv('digital_twin_predicted_curve_new_wall_poly4.csv', index=False)
...
plt.grid(True)
plt.tight_layout()
plt.savefig('pushover_curve_new_wall.png', dpi=300)
plt.show()